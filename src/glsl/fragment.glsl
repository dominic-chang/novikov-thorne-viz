...
void main() {
    ...
    if (rs > 6.0) {
        ...
        gl_FragColor += 20.0*texture2D(texture1, uv2)*get_disk_color(rs, cphi, sigma);
    ...
    if (rs1 > 6.0 && enable_grav_lensing){
        ...
        gl_FragColor += 20.0*texture2D(texture1, uv3)*get_disk_color(rs1, cphi, sigma);
    ...
    if (rs2 > 6.0 && enable_grav_lensing){
        ...
        gl_FragColor += 20.0*texture2D(texture1, uv4)*get_disk_color(rs2, cphi, sigma);
